...
if (args.length < 2) {
  ...
  console.error("");
  console.error(
    'Example: node bin/find-entry.js temp/www.facebook.com.har "hajimetechallenge"',
  );
  process.exit(1);
}
...
for (const entry of har.log.entries) {
  ...
  for (const line of lines) {
    ...
    const lineContainsKeyword =
      line.includes(keyword) || (hasNonAscii && line.includes(escapedKeyword));
